...
mid_freq_range = (250, 1000)
high_freq_range = (1000, 10000)
...
plt.plot(t[index_of_max_low], data_in_db[index_of_max_low], 'bo')
plt.plot(t[index_of_max_mid], data_in_db[index_of_max_mid], 'bo')
plt.plot(t[index_of_max_high], data_in_db[index_of_max_high], 'bo')
...
"""
def audiosegment_to_db(audiosegment):
    # Convert the AudioSegment to a NumPy array
    samples = np.array(audiosegment.get_array_of_samples())

    # Calculate the decibel values
    db_values = 20 * np.log10(np.abs(samples) / (np.iinfo(samples.dtype).max+1))

    return db_values


# Example usage
input_mp3_path = 'Thursday_at_13-43.m4a'
audio = AudioSegment.from_file(input_mp3_path, format='m4a')

# Get decibel values
db_values = audiosegment_to_db(audio)

# Plotting
samples = np.array(audio.get_array_of_samples())
duration_in_sec = len(audio) / 1000
time = np.linspace(0, duration_in_sec, num=len(samples))
plt.plot(time, db_values, color='blue')
plt.xlabel('Time (seconds)')
plt.ylabel('Decibel (dB)')
plt.title('Decibel values of the AudioSegment')
plt.grid(True)
plt.show()"""

loop = {1: 20, 2: 100, 3: 500}
...
sample_rate, data = wavfile.read('PinkPanther30.wav')
print(wavfile.read('PinkPanther30.wav'))
spectrum, freqs, t, im = plt.specgram(data, Fs=sample_rate, NFFT=1024)
plt.close()
...
data_in_db = frequency_check(1000)
...
sliced_array = data_in_db[index_of_max:]
print(f'value of max = {sliced_array}')
value_of_max_less_5 = value_of_max - 5
...
print(f'The RT60 reverb time at freq {int(target_frequency)} Hz is {np.round(abs(rt60), 3)} seconds')
